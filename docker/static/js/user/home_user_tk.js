...
function validateSize(){
    if( typeof (($("#attach"))[0].files[0]) === 'undefined'){
        var client = $('#id02 input[name=client]').val();
        $('#id02 input[name=client]').val(escapeHtml(client));
        var info_client = $('#id02 input[name=info_client]').val();
        $('#id02 input[name=info_client]').val(escapeHtml(infO_client));
        var loai_su_co = $('#id02 input[name=loai_su_co]').val();
        $('#id02 input[name=loai_su_co]').val(escapeHtml(loai_su_co));
        var thong_so_kt = $('#id02 textarea[name=thong_so_kt]').val();
        $('#id02 textarea[name=thong_so_kt]').val(escapeHtml(thong_so_kt));
        var content = $('#id02 textarea[name=content]').val();
        $('#id02 textarea[name=content]').val(escapeHtml(content));
        return true;
    }
...
$(document).ready(function(){
    ...
    var table = $('#list_tk_gui_di').DataTable({
        ...
        "ajax": {
            "type": "GET",
            "url": location.href +"gui_di"+"_"+userName,
