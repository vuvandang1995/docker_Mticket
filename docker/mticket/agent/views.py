    ...
    if request.session.has_key('agent') and (Agents.objects.get(username=request.session['agent'])).status == 1:
        ...
        for tk in Tickets.objects.filter(status=0, serviceid__in=service).order_by('-id'):
            ...
            if len(thong_so_kt_data) < 5:
                thong_so_kt = r'<p>' + tk.thong_so_kt + '</p>'
            else:
                thong_so_kt_data_5 = ''
                for s in thong_so_kt_data[:4]:
                    thong_so_kt_data_5 += s + " "
                thong_so_kt = r'<a data-toggle="collapse" data-target="#thong_so' + str(
                    tk.id) + '"><p>' + thong_so_kt_data_5 + '...</p></a><div id="thong_so' + str(
                    tk.id) + '" class="collapse"><p>' + tk.thong_so_kt + '<p></div>'
            if tk.attach != '':
            ...
            data.append([tk.id, tk.serviceid.name, loai_su_co, content, thong_so_kt,
